...
`bun run lint`, `bun run test` (53 tests), `bun run check-types` and `bun run build` all pass. The API serves
...
category is believable, in `packages/pipeline/notebooks/01_proof_laptop.ipynb`.
...
| `bun run test` | passes — 51 pipeline tests, 2 api tests |
...
implemented-flag/implementation correspondence, internal link resolution, and orphaned pages. External links
...
| `tests/` | 51 tests + a CSV fixture, so a clean clone runs green with no real data |
...
### Frontends — starter demos

`apps/web` (:3000) and `apps/dashboard` (:3001) are still unmodified `create-turbo` pages with Tailwind v4 and
shadcn wired up. `packages/ui` has three starter components.

### CI — works

`.github/workflows/ci.yml`: a JS job (bun, `turbo lint check-types build` filtered to the JS workspaces), a
